...
def model_train(new_data):
    ...
    old_data = pickle.load(open("AI/data/data.pth",'rb'))
...
def get_train(uid,train_data_set):
    model=pickle.load(open("AI/model_lib/base/{}.pth".format(gen),'rb'))
    ...
    try:
        ...
        raise e
    clear(uid)
    pickle.dump(Umodel,open("AI/model_lib/usr/{}.pth".format(uid),'wb'))
    return acc

def get_predict(uid,flow,opt=-1):
    # print("flow:",flow)
    if (opt==1):
        f=open("AI/model_lib/base/{}.pth".format(gen),"rb")
    elif (opt==0):
        Path="AI/model_lib/usr/{}.pth".format(uid)
        ...
        if (os.path.exists(Path)):
            ...
            f = open("AI/model_lib/base/{}.pth".format(gen),"rb")
    model=pickle.load(f)
    model2=pickle.load(open("AI/model_lib/base/{}.pth".format(gen),"rb"))
    flow2 = flow
